  ...
  return (
    <section id="livros" ref={ref} className="relative py-16 md:py-36 overflow-hidden bg-[#080E11]">
      <div className="absolute top-0 right-0 w-[600px] h-[600px] pointer-events-none"
        style={{background:'radial-gradient(ellipse at top right,rgba(255,200,78,0.06) 0%,transparent 65%)'}} />
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        ...
        <div className="flex flex-col gap-6 md:gap-8">
          {books.map((book, idx) => {
            ...
            return (
              <div key={book.slug}
                className={`reveal group relative flex flex-col md:flex-row ${idx % 2 === 1 ? 'md:flex-row-reverse' : ''} gap-0 border border-white/8 hover:border-[#FFC84E]/40 bg-[#0D1518] overflow-hidden transition-all duration-500 hover:shadow-[0_24px_80px_rgba(255,200,78,0.10)]`}>

                {/* Glow accent */}
                <div className="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                  style={{background:'radial-gradient(ellipse at center,rgba(255,200,78,0.04) 0%,transparent 70%)'}} />

                {/* Cover — takes ~40% width on desktop */}
                <div className="relative w-full md:w-[38%] aspect-[3/4] md:aspect-auto md:min-h-[380px] overflow-hidden bg-[#121C20] flex-shrink-0">
                  <Image
                    src={book.cover}
                    alt={book.title}
                    fill
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent md:bg-gradient-to-r md:from-transparent md:to-[#0D1518]/80" />

                  {/* Badge */}
                  {m.badge && (
                    <div className="absolute top-4 left-4">
                      <span className="font-inter text-[9px] font-bold tracking-[0.25em] uppercase bg-[#FFC84E] text-black px-2.5 py-1">
                        {m.badge}
                      </span>
                    </div>
                  )}
                ...
                <div className="relative flex flex-col justify-center gap-4 p-6 sm:p-8 md:p-10 flex-1">
                  ...
                  <div className="flex items-center justify-center md:justify-start gap-3 flex-wrap">
                    <div className="flex gap-0.5">
                      {[...Array(m.stars)].map((_,i) => (
                        <Star key={i} size={12} className="fill-[#FFC84E] text-[#FFC84E]" />
                      ))}
                    </div>
                  ...
                  {/* Title */}
                  <h3 className="font-bebas text-[clamp(26px,5vw,48px)] leading-tight text-white group-hover:text-[#FFC84E] transition-colors text-center md:text-left">
                    {book.title}
                  </h3>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 mt-2">
                    <a href={m.ctaUrl} target="_blank" rel="noopener noreferrer"
                      className="flex items-center justify-center gap-2 bg-[#FFC84E] hover:bg-[#d4a84a] active:scale-95 text-black font-inter text-[11px] font-bold tracking-[0.2em] uppercase px-7 py-4 transition-all duration-200 min-h-[52px] shadow-[0_8px_30px_rgba(255,200,78,0.25)] hover:shadow-[0_12px_40px_rgba(255,200,78,0.40)]">
                      {isWA
