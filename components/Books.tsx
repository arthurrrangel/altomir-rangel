...
function BookCard({ book }: { book: typeof books[0] }) {
  return (
    <article className="reveal group flex flex-col bg-white shadow-sm hover:shadow-xl border border-[#EDE8E0] hover:border-gold/40 transition-all duration-500 overflow-hidden">
      {/* Book cover */}
      <div className="relative h-64 overflow-hidden" style={{ background: book.cover }}>
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700"
          style={{ background: 'linear-gradient(120deg, transparent 30%, rgba(255,255,255,0.05) 50%, transparent 70%)' }} />
        ...
        <div className="absolute inset-0 flex flex-col justify-between p-7">
          <div className="flex flex-col gap-2">
            ...
            <h3 className="font-bebas text-3xl text-white leading-tight">{book.title}</h3>
            <p className="font-inter text-[11px] tracking-[0.15em] uppercase" style={{ color: book.accent }}>{book.subtitle}</p>
          </div>
          <p className="font-playfair italic text-sm text-white/35">Altomir Rangel</p>
        </div>
        <div className="absolute top-4 right-4 bg-black/50 px-2 py-0.5">
          <span className="font-inter text-[10px] text-white/40 tracking-[0.15em]">{book.pages}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col flex-1 p-7 gap-4">
        ...
        <p className="font-inter text-[#1A1305]/55 text-[13px] leading-relaxed flex-1">{book.desc}</p>
...
export default function Books() {
  ...
  return (
    <section id="livros" ref={ref} className="relative py-28 md:py-36 overflow-hidden" style={{ background: '#F5F0E8' }}>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
          <div>
            ...
            <h2 className="reveal font-bebas text-[clamp(48px,7vw,90px)] leading-none text-[#1A1305]">
              LIVROS QUE<br /><span className="text-gold">TRANSFORMAM VIDAS</span>
            </h2>
          </div>
          <p className="reveal font-inter text-[#1A1305]/45 text-sm max-w-xs leading-relaxed text-right hidden md:block">
        ...
        {/* Divider note */}
        <div className="reveal mt-12 flex items-center justify-center gap-5">
          <div className="h-px flex-1 bg-[#D8CFBC]/70" />
          <p className="font-inter text-[#1A1305]/30 text-xs tracking-[0.25em] uppercase whitespace-nowrap">
            Livros físicos · Enviamos para todo o Brasil
          </p>
          <div className="h-px flex-1 bg-[#D8CFBC]/70" />
        </div>

        {/* CTA banner — dark for contrast punch */}
        <div className="reveal mt-16 bg-[#1A1305] p-10 md:p-14 flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl">
          <div>
            <p className="font-inter text-[10px] font-semibold tracking-[0.3em] text-gold/60 uppercase mb-3">Oferta Especial</p>
            <h3 className="font-bebas text-4xl md:text-5xl text-white leading-tight">
              COMBO COMPLETO<br /><span className="text-gold">DE LIVROS</span>
            </h3>
            <p className="font-inter text-white/40 text-sm mt-3 leading-relaxed">
