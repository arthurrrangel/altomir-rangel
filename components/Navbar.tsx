...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${scrolled ? 'bg-[#080E11]/95 backdrop-blur-md border-b border-white/5' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-6 md:px-10 h-16 md:h-20 flex items-center justify-between">
        <a href="#" className="flex flex-col leading-none">
          <span className="font-bebas text-2xl text-white tracking-widest">ALTOMIR</span>
          <span className="font-inter text-[9px] text-[#FFC84E] tracking-[0.4em] uppercase">RANGEL</span>
        </a>
        <nav className="hidden md:flex items-center gap-8">
          {links.map(l => (
            <a key={l.href} href={l.href}
              className="font-inter text-[11px] font-semibold tracking-[0.25em] uppercase text-white/50 hover:text-[#FFC84E] transition-colors">
              {l.label}
            </a>
          ))}
        </nav>
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${open ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="bg-[#080E11]/98 backdrop-blur-md border-t border-white/5 px-6 py-6 flex flex-col gap-1">
          {links.map(l => (
            <a key={l.href} href={l.href} onClick={() => setOpen(false)}
              className="font-inter text-[13px] font-semibold tracking-[0.25em] uppercase text-white/60 hover:text-[#FFC84E] transition-colors py-3 border-b border-white/5 last:border-0 text-center">
              {l.label}
            </a>
          ))}
